...
map_event_type <- function(event_type) {
  ...
  if (is.null(event_type) || length(event_type) == 0) {
    return(character(0))
  }
...
load_unit_data_from_rem <- function(survey_name, unit_id) {
  if (is.null(survey_name) || is.null(unit_id)) {
    return(list())
  }
  ...
  for (unit in data) {
    ...
    for (q in unit$questionnaires) {
      ...
      if (!is.null(qd$EXTERNAL)) {
        for (key in names(qd$EXTERNAL)) {
          val <- qd$EXTERNAL[[key]]
          if (!is.null(val)) result[[key]] <- as.character(val)
        }
      }
          ...
          if (!is.null(item$COLLECTED)) {
            ...
            if (is.list(collected)) {
              ...
              for (i in seq_along(collected)) {
                val <- collected[[i]]
                vals[[i]] <- if (!is.null(val)) as.character(val) else NA
              }
              result[[key]] <- vals
            } else if (length(collected) == 1) {
              result[[key]] <- as.character(collected)
            }
          } else if (!is.null(item) && !is.list(item)) {
            result[[key]] <- as.character(item)
          }
        }
      }
    }
...
load_n1_data <- function(survey_name, unit_id) {
  ...
  for (vname in names(best_match)) {
    if (vname == "interrogationId" || vname == "ANNEE_DONNEES" ||
      ...
      grepl("^FILTRE", vname) || vname == "ESTIM") {
      next
    }
...
list_data_files <- function(survey_name) {
  ...
  base <- if (survey_name == "PrixGrumes") {
    file.path("..", "PrixGrumes", "data")
...
load_csv_data <- function(path) {
  ...
  data <- data[, !grepl("_STATE$", names(data))]
  data <- data[, !grepl("^FILTER_RESULT_", names(data))]
...
load_interrogation_statuses <- function(survey_name) {
  ...
  if (!file.exists(interog_file)) {
    parent <- file.path("..", survey_name)
    files <- list.files(parent, pattern = "^interrogations_.*\\.csv$", full.names = TRUE)
  ...
  if (!"surveyUnitId" %in% names(data)) {
    if ("usualSurveyUnitId" %in% names(data)) names(data)[names(data) == "usualSurveyUnitId"] <- "surveyUnitId" else return(NULL)
  }
  if ("highestEventType" %in% names(data)) {
    data$statut_label <- sapply(data$highestEventType, map_event_type)
  } else {
    data$statut_label <- "À expertiser"
    data$highestEventType <- NA
  }
...
get_unit_status <- function(statuses, unit_id) {
  if (is.null(statuses) || is.null(unit_id)) {
    return(list(event_type = NA, statut_label = "À expertiser", isOnProbation = FALSE))
  }
  ...
  list(
    event_type = if (!is.null(row$highestEventType)) row$highestEventType[1] else NA,
...
load_survey_units_from_rem <- function(survey_name) {
  ...
  units <- data.frame(
    id = character(), statut = character(), statut_label = character(),
  ...
  for (unit in data) {
    ...
    if (!is.null(unit$contacts) && length(unit$contacts) > 0) {
      ...
      if (!is.null(c$phoneNumbers) && length(c$phoneNumbers) > 0) {
        for (p in c$phoneNumbers) {
          if (!is.null(p$favorite) && p$favorite == TRUE && !is.null(p$number)) {
            contact_tel <- p$number
            break
          }
        }
    ...
    if (nchar(unit_id) > 0) {
      units <- rbind(units, data.frame(
        id = unit_id, statut = unit_status$event_type %||% "A_SAISIR",
  ...
  units <- units[!duplicated(units$id), ]
  units <- units[order(units$id), ]
  rownames(units) <- NULL
  units
}
...
list_survey_units <- function(survey_name) {
  ...
  if (!is.null(rem_units) && nrow(rem_units) > 0) {
    return(rem_units)
  }
  ...
  for (name in names(files)) {
    if (grepl("RACINE", name, ignore.case = TRUE)) {
      racine_file <- files[[name]]
      break
    }
  }
    ...
    for (name in names(files)) {
      ...
      if (has_module_col) {
        for (i in 1:nrow(unit_data)) {
          row <- unit_data[i, ]
          mv <- as.character(row$Q_REGIONS_ESSENCES)
...
extract_survey_dates <- function(context) {
  ...
  p <- partitions[[1]]
  list(
    label = context$label, short_label = context$shortLabel,
...
extract_survey_metadata <- function(context) {
  ...
  list(
    operation_label = if (!is.null(m$statisticalOperationSerieLabel)) m$statisticalOperationSerieLabel else NULL,
...
export_interrogations_csv <- function(survey_name, output_path = NULL) {
  ...
  export_df <- data.frame(
    partitioningId = partition_id, surveyUnitId = units$id,
